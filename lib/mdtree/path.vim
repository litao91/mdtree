...
function! s:Path.cacheDisplayString() abort
    ...
    if !empty(self._bookmarkNames) && g:MDTreeMarkBookmarks == 1
    ...
    if self.isReadOnly
        let self.cachedDisplayString .=  ' ['.g:MDTreeGlyphReadOnly.']'
    endif
endfunction
...
function! s:Path.compareTo(path)
    ...
    elseif thisSS > thatSS
        ...
        if !g:MDTreeSortHiddenFirst
            let thisPath = substitute(thisPath, '^[._]', '', '')
            let thatPath = substitute(thatPath, '^[._]', '', '')
        endif
        "if the sort sequences are the same then compare the paths
        "alphabetically
        let pathCompare = g:MDTreeCaseSensitiveSort ? thisPath <# thatPath : thisPath <? thatPath
        if pathCompare
            return -1
        else
            return 1
...
" filesystem. If the path already exists, a MDTree.Path.Exists exception is
" thrown. If any other errors occur, a MDTree.Path exception is thrown.
"
" Args:
...
function! s:Path.Create(fullpath)
    ...
    if isdirectory(a:fullpath) || filereadable(a:fullpath)
        throw "MDTree.CreatePathError: Directory Exists: '" . a:fullpath . "'"
    endif
    ...
    try
        ...
        throw "MDTree.CreatePathError: Could not create path: '" . a:fullpath . "'"
    endtry
...
function! s:Path.copy(dest)
    if !s:Path.CopyingSupported()
        throw "MDTree.CopyingNotSupportedError: Copying is not supported on this OS"
    endif

    call s:Path.createParentDirectories(a:dest)

    if exists('g:MDTreeCopyCmd')
        let cmd_prefix = g:MDTreeCopyCmd
    else
        let cmd_prefix = (self.isDirectory ? g:MDTreeCopyDirCmd : g:mdtreeCopyFileCmd)
    endif

    let cmd = cmd_prefix . " " . escape(self.str(), self._escChars()) . " " . escape(a:dest, self._escChars())
    let success = system(cmd)
    if v:shell_error != 0
        throw "MDTree.CopyError: Could not copy ''". self.str() ."'' to: '" . a:dest . "'"
    endif
endfunction
...
function! s:Path.CopyingSupported()
    return exists('g:MDTreeCopyCmd') || (exists('g:mdtreeCopyDirCmd') && exists('g:mdtreeCopyFileCmd'))
endfunction
...
"
" Throws MDTree.Path.Deletion exceptions
function! s:Path.delete()
    if self.isDirectory

        let cmd = g:MDTreeRemoveDirCmd . self.str({'escape': 1})
        let success = system(cmd)

        if v:shell_error != 0
            throw "MDTree.PathDeletionError: Could not delete directory: '" . self.str() . "'"
        endif
    else
        if exists('g:MDTreeRemoveFileCmd')
            let cmd = g:MDTreeRemoveFileCmd . self.str({'escape': 1})
            let success = system(cmd)
        else
        ...
        if success != 0
            throw "MDTree.PathDeletionError: Could not delete file: '" . self.str() . "'"
        endif
    endif

    "delete all bookmarks for this path
    for i in self.bookmarkNames()
        let bookmark = g:MDTreeBookmark.BookmarkFor(i)
...
" returns the index of the pattern in g:MDTreeSortOrder that this path matches
function! s:Path.getSortOrderIndex()
    let i = 0
    while i < len(g:MDTreeSortOrder)
        if  self.getLastPathComponent(1) =~# g:MDTreeSortOrder[i]
            return i
        endif
        let i = i + 1
    endwhile
    return s:MDTreeSortStarIndex
endfunction
...
function! s:Path.getSortKey()
    if !exists("self._sortKey")
        ...
        if !g:MDTreeCaseSensitiveSort
            let path = tolower(path)
        endif
        if !g:MDTreeNaturalSort
...
"
"
" Throws MDTree.Path.InvalidArguments exception.
function! s:Path.readInfoFromDisk(fullpath)
    ...
    if getftype(fullpath) ==# "fifo"
        throw "MDTree.InvalidFiletypeError: Cant handle FIFO files: " . a:fullpath
    endif
    ...
    elseif filereadable(a:fullpath)
        ...
        throw "MDTree.InvalidArgumentsError: Invalid path = " . a:fullpath
    endif
...
function! s:Path.rename(newPath)
    if a:newPath ==# ''
        throw "MDTree.InvalidArgumentsError: Invalid newPath for renaming = ". a:newPath
    endif

    let success =  rename(self.str(), a:newPath)
    if success != 0
        throw "MDTree.PathRenameError: Could not rename: '" . self.str() . "'" . 'to:' . a:newPath
    endif
    call self.readInfoFromDisk(a:newPath)

    for i in self.bookmarkNames()
        let b = g:MDTreeBookmark.BookmarkFor(i)
...
"  'UI' - a string to be displayed in the MDTree user interface
"
...
function! s:Path.str(...)
    ...
    if has_key(options, 'format')
        ...
        if has_key(self, '_strFor' . format)
            ...
            throw 'MDTree.UnknownFormatError: unknown format "'. format .'"'
        endif
    else
